...
objective as `23_SmellyMarsRover`, but with a more everyday domain: a shopping
cart (inspired by `20_ShoppingCart`), built across the same
...
Unlike `16_SmellyTicTacToe` (which bakes in **production code smells** to
...
Unlike `23_SmellyMarsRover`, which is a single-file fixture, this kata is
...
None of the `.cs` files in this kata contain explanatory comments about the
deliberate blockers or smells (a couple of comments do explain C#-specific
seam choices forced by the port -- see `PORTING_NOTES_CS.md` in
`ts-kata-answers` if you get curious). Finding and naming each blocker and
...
| Domain (services) | tested             | tests exist in `Tests/UnitTests.cs`, but nearly all are smelly         |
| Infrastructure    | tested             | tests exist in `Tests/IntegrationTests.cs`, but nearly all are smelly  |
...
than in `23_SmellyMarsRover`, spread across two layers.
...
2. **Fix the existing unit tests** -- everything in `Tests/UnitTests.cs`
...
3. **Fix the existing integration tests** -- same exercise, but for
   `Tests/IntegrationTests.cs`. Pay attention to how the same smell can look
...
```sh
dotnet test 24_SmellyShoppingCart
```
