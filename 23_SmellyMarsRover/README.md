...
mission simulator (inspired by `11_MarsRover` and `19_ESAMarsRover`) built
...
Neither the production classes nor the test classes in `kata.cs` contain any
explanatory comments (beyond the two callouts needed to explain a C#-specific
seam — see `PORTING_NOTES_CS.md` in `ts-kata-answers` if you get curious).
...
| `CommandTranslator`, `MissionLog`, `Coordinate`               | tested   | tests exist in `kata.cs`, but nearly all of them are smelly  |
...
sequence is written to a telemetry port (`MissionLog`).

## What to Look For

The 0%-covered part of `kata.cs` has several deliberate testability blockers —
...
The tests in `kata.cs` currently all pass, but the suite is deliberately
...
2. **Fix the existing tests** — everything in `kata.cs` currently passes, but
...
```sh
dotnet test 23_SmellyMarsRover
```
